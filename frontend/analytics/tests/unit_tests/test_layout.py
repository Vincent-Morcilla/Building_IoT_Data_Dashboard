...
def test_create_layout(mock_plot_configs, mock_categories_structure):
    ...
    assert isinstance(
        layout.children[0], dcc.Location
    ), "First child should be `dcc.Location` for URL path handling."

    sidebar = layout.children[1]
    assert isinstance(
        sidebar, html.Div
    ), "Second child should be an `html.Div` sidebar."
    assert (
        "sidebar" in sidebar.className
    ), "Sidebar `html.Div` should have `sidebar` as a class name."

    spinner = layout.children[2]
    assert isinstance(
        spinner, dbc.Spinner
    ), "Third child should be `dbc.Spinner` for loading states."
    assert isinstance(
        spinner.children[0], html.Div
    ), "Spinner should contain an `html.Div` for content display area."
    assert (
        spinner.children[0].id == "page-content"
    ), "The content `html.Div` should have id 'page-content'."


def test_home_page_content_with_categories():
    """
    Test that home_page_content returns the correct structure
    with categories available in categories_structure.
    """
    # Mock categories_structure with categories present
    mock_categories_structure = (
        {"Category1": ["Subcategory1"], "Category2": ["Main"]},
        {},
        {},
    )

    content = home_page_content(mock_categories_structure)

    assert isinstance(
        content, html.Div
    ), "home_page_content should return an `html.Div`."
    assert hasattr(content, "children") and isinstance(
    ...
    assert (
        len(content.children) == 2
    ), "The content `html.Div` should have exactly two children."

    img, paragraph = content.children[0], content.children[1]
    assert isinstance(
        img, html.Img
    ), "First child should be an `html.Img` for the logo."
    assert (
        img.src == "/assets/title-logo.svg"
    ), "The image source should be `/assets/title-logo.svg`."
    assert img.alt == "Title-Logo", "The image `alt` attribute should be 'Title-Logo'."
    assert (
        img.className == "title-logo"
    ), "The image should have the class `title-logo`."

    assert isinstance(
        paragraph, html.P
    ), "Second child should be an `html.P` for the prompt message."
    assert (
        paragraph.children == "Select an option from the sidebar categories"
    ), "Paragraph should contain the prompt 'Select an option from the sidebar categories'."


def test_home_page_content_no_categories():
    """
    Test that home_page_content returns the correct structure
    when no categories are available in categories_structure.
    """
    # Mock categories_structure with None to simulate no available categories
    content = home_page_content(None)
    ...
    assert (
        paragraph.children == "No analyses could be run on the provided dataset"
    ), "Paragraph should contain the message 'No analyses could be run on the provided dataset'."

...
def test_create_category_structure():
    ...
    categories, category_key_mapping, subcategory_key_mapping = create_category_structure(analysis_list)
    assert categories["Main Category"] == ["Sub Category"], "Subcategory mapping failed for tuple input"
    assert category_key_mapping["Main Category"] == "MainCategory", "Category key mapping failed"
    assert subcategory_key_mapping[("Main Category", "Sub Category")] == "SubCategory", "Subcategory key mapping failed"

    # Test case: Category provided without a subcategory
    analysis_list = [("MainCategory", None)]
    categories, category_key_mapping, subcategory_key_mapping = create_category_structure(analysis_list)
    assert categories["Main Category"] == ["Main"], "Default subcategory handling failed"
    assert category_key_mapping["Main Category"] == "MainCategory", "Category key mapping failed"
    assert subcategory_key_mapping[("Main Category", "Main")] is None, "Subcategory key mapping for None failed"
    ...
    assert category_key_mapping["Single Category"] == "SingleCategory", "Category key mapping failed"
    assert subcategory_key_mapping[("Single Category", "Main")] is None, "Subcategory key mapping for single category failed"

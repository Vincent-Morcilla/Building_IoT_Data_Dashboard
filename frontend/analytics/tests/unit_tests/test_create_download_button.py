...
def test_create_global_download_button_structure():
    ...
    assert isinstance(component, html.Div), "Expected top-level component to be html.Div"
    assert component.id == "global-download-container", "Top-level Div ID is incorrect"
    assert "d-grid gap-2 d-md-flex justify-content-md-end" in component.className, \
    ...
    assert len(children) == 3, "Expected 3 children: button, download component, feedback div"
    ...
    assert button.children == "Download All Dataframes as CSVs", "Button text is incorrect"
    ...
    assert isinstance(download_component, dcc.Download), "Second child should be a dcc.Download"
    assert download_component.id == "global-download-data", "Download component ID is incorrect"
...
def test_create_global_download_button_components(component_index, expected_type, expected_id):
    """
    ...
    assert isinstance(child, expected_type), f"Expected type {expected_type} at index {component_index}"
    assert child.id == expected_id, f"Expected ID '{expected_id}' at index {component_index}"
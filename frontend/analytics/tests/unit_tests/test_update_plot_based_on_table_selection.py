...
def test_update_plot_based_on_table_selection():
    ...
    """
    # Sample table data
    table_data = pd.DataFrame({"ID": [1, 2, 3], "Name": ["Room A", "Room B", "Room C"]})
    ...
    # Outputs
    outputs = [
        {"component_id": "updateable-line-chart", "component_property": "figure"}
    ]
    ...
    for trace, column in zip(
        output_results[0].data, ["Value", "Variable1", "Variable2"]
    ):
        assert (
            trace.x.tolist() == expected_df["Date"].tolist()
        ), f"X-axis for {column} is incorrect."
        assert (
            trace.y.tolist() == expected_df[column].tolist()
        ), f"Y-axis for {column} is incorrect."

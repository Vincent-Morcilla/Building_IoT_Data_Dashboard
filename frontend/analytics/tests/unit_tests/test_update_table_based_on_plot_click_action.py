...
def test_update_table_based_on_plot_click_action():
    ...
    input_values = [False]
    outputs = [{'component_id': 'inconsistent-classes-table', 'component_property': 'data'}]
    ...
    assert isinstance(output_results[0], list), "Expected output to be a list of records"
    ...
    assert output_results[0] == expected_df.to_dict('records'), "Filtered data does not match expected result"

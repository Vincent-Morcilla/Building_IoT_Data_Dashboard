...
def apply_transformation(
    ...
    if transformation_type == 'aggregate':
        ...
        if groupby_columns and aggregations:
            transformed_df = transformed_df.groupby(groupby_columns).agg(**aggregations).reset_index()
        ...
        if on_column in transformed_df.columns:
            transformed_df[on_column] = pd.to_datetime(transformed_df[on_column]) # Ensure datetime
            transformed_df.set_index(on_column, inplace=True)
            transformed_df = transformed_df.resample(frequency).agg(agg_func).reset_index()

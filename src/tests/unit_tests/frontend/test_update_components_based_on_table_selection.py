...
def test_table_data_is_none(setup_data):
    """
    Test when table_data is None in data_source.
    """
    setup_data["interaction"]["data_source"]["table_data"] = None
    input_values = [[0]]  # selected_rows
    ...
    assert "Data source or index column not found in interaction configuration." in str(
        exc_info.value
    )


def test_data_dict_is_none(setup_data):
    """
    Test when data_dict is None in data_source.
    """
    setup_data["interaction"]["data_source"]["data_dict"] = None
    input_values = [[0]]  # selected_rows
...
def test_index_column_is_none(setup_data):
    """
    Test when index_column is None in interaction.
    """
    setup_data["interaction"]["index_column"] = None
    input_values = [[0]]  # selected_rows
...
def test_component_type_table(setup_data):
    """
    Test when component type is 'table'.
    """
    setup_data["data_dict"]["stream1"] = [{"type": "table", "id": "table1"}]
    input_values = [[0]]  # selected_rows
    ...
    with patch(
        ...
        component = result[0][0]
        assert isinstance(component, html.Div)
        assert component.children == "Table Component"


def test_component_type_UI(setup_data):
    """
    Test when component type is 'UI'.
    """
    setup_data["data_dict"]["stream1"] = [{"type": "UI", "id": "ui1"}]
    input_values = [[0]]  # selected_rows

    with patch(
        "actions.update_components_based_on_table_selection.create_ui_component"
    ) as mock_create_ui_component:
        mock_create_ui_component.return_value = html.Div("UI Component")
        result = update_components_based_on_table_selection_action(
            setup_data["plot_configs"],
            input_values,
        ...
        component = result[0][0]
        assert isinstance(component, html.Div)
        assert component.children == "UI Component"


def test_component_type_separator(setup_data):
    """
    Test when component type is 'separator'.
    """
    setup_data["data_dict"]["stream1"] = [
        {"type": "separator", "style": {"margin": "10px"}}
    ]
    input_values = [[0]]  # selected_rows
    ...
    assert len(result[0]) == 1
    component = result[0][0]
    assert isinstance(component, html.Hr)
    assert component.style == {"margin": "10px"}


def test_unsupported_component_type(setup_data):
    """
    Test when component type is unsupported.
    """
    setup_data["data_dict"]["stream1"] = [{"type": "unsupported", "id": "comp1"}]
    input_values = [[0]]  # selected_rows
    ...
    assert "Unsupported component type 'unsupported' in data_dict." in str(
        exc_info.value
    )

...
def test_pascal_to_words():
    ...
    assert (
        pascal_to_words("PascalCaseText") == "Pascal Case Text"
    ), "Failed on standard PascalCase input"
    ...
    assert (
        pascal_to_words("nocamelcase") == "nocamelcase"
    ), "Failed on input without PascalCase"

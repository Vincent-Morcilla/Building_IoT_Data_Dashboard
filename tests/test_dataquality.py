...
def test_data_quality():
    ...
    try:
        db = DBManager(
            str(DATA_ZIP_PATH), str(MAPPER_PATH), str(MODEL_PATH), str(SCHEMA_PATH)
        )
    ...
    try:
        ...
        assert (
            "DataQualityTable" in results
        ), "Data quality table is missing from results"
        ...
        assert (
            results["DataQualityTable"]["type"] == "table"
        ), "Data quality result should be a table"
        assert (
            "header" in results["DataQualityTable"]
        ), "Data quality table should have a header"
        assert (
            "cells" in results["DataQualityTable"]
        ), "Data quality table should have cells"

        # Similar checks for the summary table
        assert (
            results["SummaryTable"]["type"] == "table"
        ), "Summary result should be a table"
        ...
        assert (
            output_dir / "data_quality_table.csv"
        ).exists(), "Data quality CSV file not created"
        assert (
            output_dir / "summary_table.csv"
        ).exists(), "Summary table CSV file not created"
